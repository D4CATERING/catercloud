...
function toggleSandwichOPulguita(refId, tipo) {
    ...
    if (tipo === 'sandwich') {
        if (contSandwich) contSandwich.style.display = 'block';
        if (contPulguita) contPulguita.style.display = 'none';
        if (ref) {
            ref.modo = 'sandwich';
            ref.pulguita = '';
        }
    } else {
        if (contSandwich) contSandwich.style.display = 'none';
        if (contPulguita) contPulguita.style.display = 'block';
        if (ref) {
            ref.modo = 'pulguita';
            ref.sandwiches = [];
        }
...
function obtenerMaximoOpcionesDesayuno(ref) {
    ...
    return Math.max(1, Math.round(Number.isFinite(cantidad) && cantidad > 0 ? cantidad : 1));
}
...
window.abrirModalDesayunoSandwichMultiple = function(refId) {
    ...
    if (!ref) return;
    const opciones = ref.opcionesDisponibles || [];
    const maxSelecciones = ref.cantidadSandwiches || ref.sandwiches?.length || 2;
    const seleccionadas = obtenerSaboresSandwichMultiple(ref).slice(0, maxSelecciones);
    sincronizarSandwichesMultiples(ref, seleccionadas);
    const body = opciones.map(opcion => `
        <label style="display:flex;align-items:center;gap:10px;padding:8px 12px;margin-bottom:4px;
                      border-radius:6px;border:1px solid #E8E6E1;cursor:pointer;
                      background:${seleccionadas.includes(opcion) ? 'rgba(219,234,254,0.5)' : 'white'}">
            <input type="checkbox" value="${opcion}" ${seleccionadas.includes(opcion) ? 'checked' : ''}
                   onchange="toggleSandwichMultipleModalCheck('${refId}','${opcion}',this.checked,this)">
            <span style="font-size:0.82rem">${opcion}</span>
        </label>`).join('');
    _abrirModalDesayuno(
        'Elegir sabores',
...
window.toggleSandwichMultipleModalCheck = function(refId, opcion, checked, el) {
    const ref = window.referenciasDesayuno?.[refId];
    if (!ref) return;
    const maxSelecciones = ref.cantidadSandwiches || ref.sandwiches?.length || 2;
    let seleccionadas = obtenerSaboresSandwichMultiple(ref);

    if (checked && !seleccionadas.includes(opcion)) {
        if (seleccionadas.length >= maxSelecciones) {
            el.checked = false;
            return;
        }
        seleccionadas.push(opcion);
    } else if (!checked) {
        seleccionadas = seleccionadas.filter(sabor => sabor !== opcion);
    }

    seleccionadas = [...new Set(seleccionadas)].slice(0, maxSelecciones);
    sincronizarSandwichesMultiples(ref, seleccionadas);

    document.querySelectorAll('#modalDesayunoBody input[type="checkbox"]').forEach(input => {
        const activo = seleccionadas.includes(input.value);
        input.checked = activo;
        const label = input.closest('label');
...
window.abrirModalDesayunoSandwichOPulguita = function(refId) {
    ...
    const maxSandwiches = 2;
    const sandwichesSeleccionados = obtenerSaboresSandwichMultiple(ref).slice(0, maxSandwiches);
    if (tipo === 'sandwich') sincronizarSandwichesMultiples(ref, sandwichesSeleccionados);
        ...
        ? `
            ...
            <div style="display:flex;flex-direction:column;gap:2px;">
                ${opciones.map(o => `
                    <label style="display:flex;align-items:center;gap:10px;padding:8px 12px;margin-bottom:4px;
                                  border-radius:6px;border:1px solid #E8E6E1;cursor:pointer;
                                  background:${sandwichesSeleccionados.includes(o) ? 'rgba(219,234,254,0.5)' : 'white'}">
                        <input type="checkbox" value="${o}" ${sandwichesSeleccionados.includes(o) ? 'checked' : ''}
                               onchange="toggleSOPSandwichModalCheck('${refId}','${o}',this.checked,this)">
                        <span style="font-size:0.82rem">${o}</span>
                    </label>`).join('')}
            </div>`
        : pulguitas.map(o => `
...
window.seleccionarSOPSandwichModal = function(refId, num, opcion) {
    ...
    const ref = window.referenciasDesayuno?.[refId];
    const completos = ref?.sandwiches?.every(s => s.sabor);
    const btn = document.getElementById('dropdown-text-' + refId);
    if (btn && ref) {
...
window.toggleSOPSandwichModalCheck = function(refId, opcion, checked, el) {
    const ref = window.referenciasDesayuno?.[refId];
    if (!ref) return;
    const maxSelecciones = 2;
    let seleccionadas = obtenerSaboresSandwichMultiple(ref);
