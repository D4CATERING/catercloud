-- Actualizacion SEGURA de catalogo public.logistics_materials
-- Esta version evita el error de duplicate key por id.
begin;

create extension if not exists pgcrypto;

-- Necesario para poder insertar/actualizar filas nuevas por item_id.
create unique index if not exists logistics_materials_item_id_key on public.logistics_materials (item_id);

-- 1) Items principales que ya tienen id: actualizar por id
...
-- 2) Items principales nuevos sin id: crear/actualizar por item_id
insert into public.logistics_materials
  (id, item_id, tipo, nombre, unidad, activo, orden, parent_id, tiene_subitems, solo_loza, solo_desechable)
values
...
-- 3) Subitems que ya tienen id: actualizar por id
insert into public.logistics_materials
  (id, item_id, tipo, nombre, unidad, activo, orden, parent_id, tiene_subitems, solo_loza, solo_desechable)
values
...
-- 4) Subitems nuevos sin id: crear/actualizar por item_id
insert into public.logistics_materials
  (id, item_id, tipo, nombre, unidad, activo, orden, parent_id, tiene_subitems, solo_loza, solo_desechable)
values
