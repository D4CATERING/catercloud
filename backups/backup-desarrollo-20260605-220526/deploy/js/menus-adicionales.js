...
(function () {
  ...
  function claveMaterialAcumulado(item, tipo) {
    ...
    const unidad = limpiarTextoMaterial(item?.unidad || '');
  ...
  function normalizarItemMaterial(item, tipo) {
    ...
    if (claveMaterialAcumulado(copia, tipo) === 'bebidas:zumo-naranja-natural') {
      copia.nombre = 'Zumo natural';
      copia.unidad = copia.unidad || 'litro';
    }
    return copia;
  }
    ...
    ['bebidas', 'menaje', 'extras'].forEach(tipo => {
      (material?.[tipo] || []).forEach(item => {
        ...
        if (existe) {
          ...
          existe.checked = existe.checked !== false && itemNormalizado.checked !== false;
  ...
  function detalleReferenciasResumen(menu) {
    ...
    if (saladas.length) {
      html += renderFilaDetalleResumen(`Saladas${mul.saladas ? ` x${mul.saladas}` : ''}`, '', true);
      saladas.forEach(ref => {
    ...
    if (postres.length) {
      html += renderFilaDetalleResumen(`Postres${mul.postres ? ` x${mul.postres}` : ''}`, '', true);
      postres.forEach(ref => {
        html += renderFilaDetalleResumen(ref.nombre || ref.id || 'Referencia', formatoCantidadResumen(ref, 'uds'), false);
      });
    }
  ...
  function detalleFoodboxResumen(menu) {
    ...
    grupos.forEach(grupo => {
      html += renderFilaDetalleResumen(grupo.label, '', true);
